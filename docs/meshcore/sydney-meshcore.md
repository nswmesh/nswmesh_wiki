...
**Encryption**
MeshCore uses **AES-256-CTR** encryption for securing communications. This means that for `Channels` And `Direct Messages` with secured keys and trustworthy recipients, your data is more that safe.
...
Having all the repeaters advertising too fast will cause mesh congestion, so longer intervals are necessary to prevent too much traffic.
